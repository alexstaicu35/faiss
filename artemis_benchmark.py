#!/usr/bin/env python3
# Artemis benchmark runner for FAISS.
#
# Uses SIFT1M by default — 1 million real 128-dim SIFT image descriptors,
# the standard ANN benchmark dataset. Falls back to FAISS's own
# SyntheticDataset if SIFT1M is not available.
#
# Set SIFT1M_DIR to override the default data path:
#   SIFT1M_DIR=/path/to/sift/ python artemis_benchmark.py
#
...
def load_sift1m(sift_dir):
    xb = fvecs_read(os.path.join(sift_dir, "sift_base.fvecs"))        # 1M x 128
    xq = fvecs_read(os.path.join(sift_dir, "sift_query.fvecs"))       # 10k x 128
    xt = fvecs_read(os.path.join(sift_dir, "sift_learn.fvecs"))       # 100k x 128
    gt = ivecs_read(os.path.join(sift_dir, "sift_groundtruth.ivecs")) # 10k x 100
    return xb, xq, xt, gt


def load_synthetic(d=128, nb=200_000, nq=1_000, nt=50_000):
    """FAISS SyntheticDataset: low-dimensional manifold embedded in d-space."""
    sys.path.insert(0, SCRIPT_DIR)
    from contrib.datasets import SyntheticDataset
    ds = SyntheticDataset(d=d, nb=nb, nq=nq, nt=nt)
    xb = ds.get_database()
    xq = ds.get_queries()
    xt = ds.get_train()
...
def load_dataset():
    if os.path.isdir(SIFT1M_DIR) and os.path.exists(
        ...
        xb, xq, xt, gt = load_sift1m(SIFT1M_DIR)
        # Use a 10k query subset to keep benchmark time reasonable
        xq = xq[:1_000]
        gt = gt[:1_000]
        return xb, xq, xt, gt, "SIFT1M"
    else:
        print("SIFT1M not found, falling back to SyntheticDataset")
...
def ms_per_query(index, xq, k=K, min_seconds=1.0):
    """Repeat until min_seconds wall time, return average ms/query."""
...
def main():
    ...
    xb, xq, xt, gt, dataset_name = load_dataset()
    print(f"Dataset: {dataset_name}  xb={xb.shape}  xq={xq.shape}")
